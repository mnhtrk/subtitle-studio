...
async fn fix_names_and_style(
    ...
    // Для русского языка добавим определение пола говорящего
    let mut processed_segments = segments.clone(); // ← Клонируем для безопасного использования
    
    if target_language == "ru" {
        // Анализируем каждый сегмент на предмет грамматических ошибок по роду
        for segment in &mut processed_segments {
            if needs_gender_correction(&segment.text) {
                // Здесь можно интегрировать определение пола из аудио
                // Но для простоты пока используем эвристический подход
                segment.text = apply_gender_correction(&segment.text);
            }
        }
    }
    
    // Объединяем все сегменты в один текст для обработки
    let full_text = processed_segments
        .iter()
        .map(|s| s.text.clone())
        .collect::<Vec<_>>()
        .join(" ");
    ...
    // Разбиваем исправленный текст обратно на сегменты
    ...
    // Создаём копию для проверки длины
    let original_len = processed_segments.len();
    ...
    for (i, mut segment) in processed_segments.into_iter().enumerate() {
        if sentence_index < corrected_sentences.len() {
            ...
            if i < original_len - 1 { // ← Используем сохранённую длину
...
fn needs_gender_correction(text: &str) -> bool {
    text.contains("виделА") || 
    text.contains("сказалА") || 
    text.contains("делалА") || 
    text.contains("хотелА") ||
    text.contains("шелА") ||
    text.contains("пришлА")
}

fn apply_gender_correction(text: &str) -> String {
    text.replace("виделА", "видел")
        .replace("сказалА", "сказал")
        .replace("делалА", "делал")
        .replace("хотелА", "хотел")
        .replace("шелА", "шел")
        .replace("пришлА", "пришел")
}